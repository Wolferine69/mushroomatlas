...
class BulkDeleteTrashMessagesTest(TestCase):
    def setUp(self):
        ...
        self.message1 = Message.objects.create(sender=self.user1, receiver=self.user2, subject='Test 1', content='Message 1')
        self.message2 = Message.objects.create(sender=self.user1, receiver=self.user2, subject='Test 2', content='Message 2', is_trashed_by_receiver=True)
    ...
    def test_bulk_delete_trash_messages(self):
        ...
        response = self.client.post(reverse('bulk_delete_trash_messages'), {'message_ids': [self.message1.pk, self.message2.pk]})
...
class BulkRestoreTrashMessagesTest(TestCase):
    def setUp(self):
        ...
        self.user1 = User.objects.create_user(username='user1', password='pass')
        self.user2 = User.objects.create_user(username='user2', password='pass')
        self.message1 = Message.objects.create(sender=self.user1, receiver=self.user2, subject='Test 1', content='Message 1', is_trashed_by_receiver=True)
        self.message2 = Message.objects.create(sender=self.user1, receiver=self.user2, subject='Test 2', content='Message 2', is_trashed_by_receiver=True)
    ...
    def test_bulk_restore_trash_messages(self):
        self.client.login(username='user2', password='pass')
        logger.debug("User2 logged in")
        response = self.client.post(reverse('bulk_restore_trash_messages'), {'message_ids': [self.message1.pk, self.message2.pk]})
...
class ViewMessageDetailTest(TestCase):
    def setUp(self):
        ...
        self.user1 = User.objects.create_user(username='user1', password='pass')
        self.user2 = User.objects.create_user(username='user2', password='pass')
        self.message = Message.objects.create(sender=self.user1, receiver=self.user2, subject='Test Subject', content='Message')
